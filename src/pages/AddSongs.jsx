    ...
    try{
      ...
      const response = await axios.post(`${url}/api/song/add`,formData); 
         ...
         if (response.data.success) {
            ...
            toast.error(response.data.message || "Failed to add song. Please try again.");
          }                                                                                                                                                                                                                                                                                                                            
    } catch{
  ...
  return loading ? (
    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col items-start text-gray-600 gap-8'>
      <div className='flex gap-8'>
        ...
        <div className='flex flex-col gap-4'>
          <p>Upload Image</p>
          <input onChange={(e) => { const file = e.target.files[0]; setImage(file); }} type="file" className='hidden' accept='image/*' id="image" />
          <label htmlFor="image">
      ...
      <div className='flex flex-col gap-2.5'>
        <p>Album</p>
        <select onChange={(e)=>setAlbum(e.target.value)} value={album} className='bg-transparent text-black outline-blue-900 border-2 border-gray-400 p-2.5 w-37.5'>
